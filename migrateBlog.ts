...
function splitRenderedHtmlIntoSegments(
  ...
  for (const node of topNodes) {
    ...
    // 1) FIGURE (img + optional figcaption)
    if ($node.is('figure')) {
    ...
    // 2) P that is purely an image wrapper
    ...
    // 4) Generic text block (keep full outerHTML)
...
async function migrateBlog() {
  ...
  const xmlData = fs.readFileSync('./blogWPData.xml', 'utf8')
  ...
  for (const blogPost of blogPosts) {
    ...
    const newBlogPostData: Partial<BlogPost> = {
      ...
      // categories: Array.isArray(blogPost.category)
      //   ? blogPost.category.map((cat) => categoryMap[cat])
      //   : [categoryMap[blogPost.category]],
    }
